...
# 1) PHP lint
echo "== PHP lint =="
php -v > ../reports/php-version.txt || true
find . -name '*.php' -not -path './vendor/*' -print0 | xargs -0 -n 1 -P 4 php -l \
  | tee ../reports/php-lint.txt
...
if [ -f phpcs.xml.dist ] && [ -d vendor ]; then
  echo "== PHPCS =="
  ./vendor/bin/phpcs -q --report=full | tee ../reports/phpcs.txt || true
else
  echo "PHPCS skipped (missing phpcs.xml.dist or vendor)" | tee ../reports/phpcs.txt
fi

# 4) PHPStan (if phpstan.neon.dist present)
if [ -f phpstan.neon.dist ] && [ -d vendor ]; then
  echo "== PHPStan =="
  ./vendor/bin/phpstan analyse -c phpstan.neon.dist | tee ../reports/phpstan.txt || true
else
  echo "PHPStan skipped (missing phpstan.neon.dist or vendor)" | tee ../reports/phpstan.txt
fi
...
../bin/install-wp-tests.sh | tee ../reports/wp-tests-install.txt || true

echo "== PHPUnit =="
if [ -f phpunit.xml.dist ]; then
  ./vendor/bin/phpunit -c phpunit.xml.dist | tee ../reports/phpunit.txt || true
else
  echo "PHPUnit skipped (missing phpunit.xml.dist)" | tee ../reports/phpunit.txt
fi
...
checks = [
  ("php_lint", "../reports/php-lint.txt"),
  ("phpcs", "../reports/phpcs.txt"),
  ("phpstan", "../reports/phpstan.txt"),
  ("phpunit", "../reports/phpunit.txt"),
]
...
pathlib.Path("../reports/gate.json").write_text(json.dumps(gate, indent=2))
print("Wrote ../reports/gate.json:", gate)
PY
